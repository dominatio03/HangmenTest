...
public class Hangman
{
	...
	public void playHangman()
	{
		...
		while (userNotDone)
		{
			...
			hasUserWon = letUserGuessWord(getWord(), getWord_form(), NUM_LIVES);

			// print result of game depending on the value of hasUserWon
			System.out.println(hasUserWon ? getWord() + "\nWell done!"
	...
	public String initialiseRound()
	{
		...
		setWord(chooseWord(getWORDS()));

		// Transform it to the *-form **...* to be shown to the user.
		setWord_form(starForm(getWord()));
	...
	public char input_y_n_Char()
	{
		...
		boolean flag = true;
		while (flag == true) {
			System.out.print("Will user play again?");
			c = inputChar();
			if (c == 'y' || c == 'n') { flag = false;}
		}
		// TODO Auto-generated method stub
	...
	public boolean occursAsNew(char c, String word, String word_form)
	{
		...
		if (word.contains(s) && !word_form.contains(s)) {
			return occurs(c, word);
		} else {
			return occurs(c, word_form);
		}
		// TODO Auto-generated method stub
	...
	public char inputChar()
	{
		...
		while (flag == true) {
			String s = input_next();																																	
			if (s.length() > 1) {System.out.print("Please enter only single letter");}
			else {c = s.charAt(0); flag = false;}
			}
		// TODO Auto-generated method stub
		return c; // Not yet implemented!

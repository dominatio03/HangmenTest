...
public class HangmanTest
{
	...
	public void testInitialiseRound()
	{
		...
		assertTrue(Arrays.asList(hangman.getWORDS()).contains(r));

		assertNotNull(hangman.getWord());

		assertNotNull(hangman.getWord_form());

		assertTrue(hangman.getWord().length() == hangman.getWord_form().length());
		
		for (int i = 0; i < hangman.getWord_form().length(); i++) { 
			assertEquals('*', hangman.getWord_form().charAt(i)); 
			} 
		
		assertTrue(hangman.getUnluckyGuesses().size() == 0);
	...
	public void testInput_y_n_Char()
	{
		...
		for (int i = 1; i <= 5; i++) { 
			System.out.println("the " + i + "th y"); 
			assertEquals('y', hangman.input_y_n_Char()); } 
		System.out.println("there is no y, so next should be n"); 
		assertEquals('n', hangman.input_y_n_Char());
	...
	public void testChooseWord()
	{
		...
		String word = hangman.chooseWord(hangman.getWORDS()); 
		assertTrue(Arrays.asList(hangman.getWORDS()).contains(word));
	...
	public void testStarForm()
	{
		...
		assertEquals("******", hangman.starForm("abadaf")); 
		assertEquals("************", hangman.starForm("erythropoeia")); 
		
		
		assertEquals("**", hangman.starForm("ab")); 
		assertEquals("***", hangman.starForm("abc")); 
		assertEquals("****", hangman.starForm("abbc"));
	...
	public void testOccurs()
	{
		...
		assertTrue("occurs failed???", hangman.occurs('c', "abbdmkc"));
	...
	public void testOccursAsNew()
	{
		...
		//fail("Not yet implemented");
		assertTrue(hangman.occursAsNew('o', "good", "g**d")); 
		assertTrue(hangman.occursAsNew('d', "good", "goo*")); 
		assertFalse(hangman.occursAsNew('g', "good", "goo*")); 
		assertFalse(hangman.occursAsNew('w', "good", "goo*"));
	...
	public void testAmend()
	{
		...
		assertEquals("aab*", hangman.amend("aabd", "**b*", 'a'));
		assertEquals("***d", hangman.amend("abcd", "****", 'd'));
	...
	public void testInputChar()
	{
		...
		for (int i = 0; i < hangman.getINPUT().size(); i++) { 
			if (hangman.getINPUT().get(i).length() == 1) { 
				assertEquals(hangman.getINPUT().get(i).charAt(0), hangman.inputChar()); 
				} 
			}
	...
	public void testInput_next()
	{
		...
		//fail("Not yet implemented");
		for (int i = 0; i < hangman.getINPUT().size(); i++) { 
			assertEquals(hangman.getINPUT().get(i), hangman.input_next()); 
			} 
		for (int i = 0; i < 3; i++) { 
			assertEquals("n", hangman.input_next()); 
			} 
